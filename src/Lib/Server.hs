...
import Lib.Error (_NotFoundError, errorMessage)
...
notFoundHandler env = Tagged $ \req res -> do
  ...
  liftIO . runLoggerT env . withNamespaceKatip "notFound" . withContextKatip (object ["method" .= method, "route" .= route'])
    $ infoKatip
    $ errorMessage appErr
  res . responseServerError . toHttpError req $ appErr
  where
  ...
  where
    ...
    contentTypeAdjuster middleware = \req res ->
      let headers = requestHeaders req
          ...
          adjustedHeaders =
            if
              | contentTypeIsJson && not acceptIsJson -> headersNoAccept <> [(hAccept, "application/json")]
